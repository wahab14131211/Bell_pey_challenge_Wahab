...
def wahab_predwar (x,y,z,b,r):
    #The b value becomes more representative of the opponents actions as the rounds progress.
        #Compare the scaled version of r to a random number to determine if the player can be sufficiently confident in the accuracy of b
        ...
        #The function to calculate this will return a value from 0.5 to 4. So, if the value is small, cooperate.
        #for any individual round, the player gets the most tokens by choosing to betray. Since this branch will be triggered in the later stages of the game where the benefits from cooperating start to diminish, this check will (on average) choose to betray
    ...
    if (r**1.1)>randint(1,10000):                   #raise r to the power of 1.1 so that by round ~5000, the player is sufficiently confident in the validity of b
